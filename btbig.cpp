...
int timten(SV a[],int n){
	...
	int dem=0,b=0;
		...
		gets(h);
	for(int i=0;i<n;i++){
		if(strcmp(h,a[i].ten)==0){
			dem++;
			b++;
		}
	}
	if(b++){
	    printf("có %d hoc sinh giong voi ten can tim\n",dem);
    }
    else{
    	printf("khong co hoc sinh nao giong voi ten can tim");
	}
}
void xuatFile(SV a[], int n, char fileName[]){
    FILE * fp;
